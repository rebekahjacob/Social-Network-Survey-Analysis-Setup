...
When I pull in the data, right away, I go ahead and remove any "preview" surveys done during testing. Consider removing any other data that doesn't belong (e.g. those that declined to give consent, not eligible, etc.)

I load survey data first since we'll use some of it's info in the attributes section.
```{r}
survey<- read_survey("SNA-template-survey-fellow.csv") %>%
  filter(Status!="Survey Preview") %>% #don't include test surveys
